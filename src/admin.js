...
function registerAdmin(bot, sendMainMenu) {
  ...
  bot.action('adm:settings', async (ctx) => {
    ...
    await ctx.reply(
      ...
      '\u2139\ufe0f Narxlar data/settings.json faylida tahrirlanadi.',
  ...
  bot.action(/^adm:msg:(welcome|sub)$/, async (ctx) => {
    ...
    await ctx.reply(
      ...
      '\ud83d\udc8e <b>Premium emojilarni bemalol ishlating</b> \u2014 ular foydalanuvchilarga ' +
      'xuddi shunday harakatli ko\u2018rinadi!\n\n' +
      '\u26a0\ufe0f Muhim: yuborgan xabaringizni bu chatdan O\u2018CHIRMANG \u2014 bot uni nusxalab ishlatadi.\n' +
      'Default holatga qaytarish uchun: <code>default</code> deb yozing.',
      { parse_mode: 'HTML' }
    );
  });
}
...
async function handleText(ctx, s) {
  ...
  if (s.action === 'admin:setmsg') {
    if (text.toLowerCase() === 'default') {
      await db.updateSettings({ [s.kind === 'welcome' ? 'welcomeMsg' : 'subMsg']: null });
      state.clear(ctx.from.id);
...
async function captureMessage(ctx, s) {
  ...
  await db.updateSettings({ [s.kind === 'welcome' ? 'welcomeMsg' : 'subMsg']: ref });
  state.clear(ctx.from.id);
  await ctx.reply(
    '\u2705 Saqlandi! Quyida foydalanuvchi ko\u2018radigan ko\u2018rinish:'
  );
  // Preview — premium emojilar bilan birga nusxalanadi
