...
async function showSubPrompt(ctx, lang, res) {
    ...
    if (sm && sm.chatId && sm.messageId) {
        try {
            // FORWARD ishlatiladi: premium emojilar harakatli holda yetib boradi
            // (copyMessage'da Telegram bot nomidan yuborilgani uchun ularni oddiy
            // emojiga almashtirib yuboradi)
            await ctx.telegram.forwardMessage(ctx.chat.id, sm.chatId, sm.messageId);
            await ctx.reply(t(lang, 'sub_required'), { parse_mode: 'HTML', ...kb });
            return;
        } catch (err) {
            console.error('subMsg forward xato:', err.message);
        }
    }
    await ctx.reply(t(lang, 'sub_required'), { parse_mode: 'HTML', ...kb });
}
